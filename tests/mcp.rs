...
fn mcp_initialize_returns_server_info() {
    ...
    let body = json_rpc_request(1, "initialize", serde_json::json!({
    ...
    assert!(stdout.contains("serverInfo"), "expected serverInfo, got: {stdout}");
    assert!(stdout.contains("\"name\":\"aum\""), "expected name=aum, got: {stdout}");
    assert!(stdout.contains("\"title\":\"agent-usage-monitor\""), "expected title, got: {stdout}");
}
...
fn mcp_initialize_then_list_tools_returns_valid_response() {
    ...
    let mut stdin = child.stdin.take().expect("stdin");
    // initialize
    let init = json_rpc_request(1, "initialize", serde_json::json!({
        "protocolVersion": "2024-11-05",
        "capabilities": {},
        "clientInfo": { "name": "test-client", "version": "0.0.1" }
    }));
...
fn mcp_call_get_quota_does_not_panic() {
    ...
    let notif = json_rpc_notification("notifications/initialized", serde_json::json!({}));
    writeln!(stdin, "{notif}").expect("write initialized");
    let call = json_rpc_request(2, "tools/call", serde_json::json!({
        "name": "get_quota",
        "arguments": {}
    }));
