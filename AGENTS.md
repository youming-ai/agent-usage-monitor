...
- **Platform Registry**: Runtime platform metadata (labels, colors, availability rules, default paths, reader factories, and quota/account capabilities) is managed centrally in `src/platforms.rs` (`REGISTRY`). Adding a platform requires a registry entry plus the explicit `Platform`, CLI, and config schema variants, but does not require orchestration changes in `main.rs`. Quota-only platforms (no local log reader, e.g. Grok) set `create_reader: None`; they are surfaced in the TUI via `platforms::displayable_platforms` and skipped by `start_watchers` (nothing to watch).
- **TUI & UI Rendering**: The TUI runs on its own thread, utilizing `std::sync::RwLock`'s `try_read()` method to fetch state from `AppState`. If a lock conflict occurs, the frame is skipped to prevent any interface lag. Available platforms are stacked vertically (no tabs) in `src/ui/mod.rs`, each rendering a header, optional live-quota panel, local-activity label, daily contribution heatmap, and an overview when height permits.
- **Data Flow**:
...
- **Default Paths**: Each agent's default data directory is defined once in the platform registry (`src/platforms.rs`); `Platform::default_path` and config defaults are thin shims around it.
